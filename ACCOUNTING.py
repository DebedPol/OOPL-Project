...
class SeriPayroll:
    ...
    def create_label(self, text, x, y, font_weight="normal"):
        label = tk.Label(self.frame, text=text, font=('Arial', 10, font_weight), fg='blue violet', bg='#ffcccb')
        label.place(x=x, y=y)
        return label
...
frame = tk.Frame(window, width=1500, height=800, bg='#ffcccb')
frame.place(x=1, y=1)
seri_payroll = SeriPayroll(frame)

# Big Title Label
big_title_label = tk.Label(frame, text="ACCOUNTING DEPARTMENT | ", font=('Arial', 30, 'bold'),fg='dark violet', bg='#ffcccb')
big_title_label.place(x=400, y=11)
big_title_label = tk.Label(frame, text=" PAYROLL MANAGEMENT ", font=('Arial', 25, 'bold'),fg='dark violet', bg='#ffcccb')
...
search_button = seri_payroll.create_button("Search", 650, 180, '#ff6347', '#4169E1', 10)
search_button.config(command=search_employee_info)  #search function to button

save_button = seri_payroll.create_button("Save", 650, 210, '#ff6347', '#4169E1', 10)
save_button.config(command=save_employee)  #save function to button

update_button = seri_payroll.create_button("Update", 650, 240, '#ff6347', '#4169E1', 10)
update_button.config(command=update_employee_info)  #update function to button

compute_button = seri_payroll.create_button("Compute", 650, 360, '#ff6347', '#4169E1', 10)
